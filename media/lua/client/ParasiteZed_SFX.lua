...
ParasiteZed.sfxZedList = {
	'ParasiteZed_Buzz1',
	'ParasiteZed_Buzz2',
	'ParasiteZed_Confused',
	'ParasiteZed_Distant',
	--'ParasiteZed_Hurt',
	'ParasiteZed_Phrase1',
	'ParasiteZed_Phrase2',
	'ParasiteZed_Phrase3',
	'ParasiteZed_Phrase4',
	'ParasiteZed_Stutter1',
	'ParasiteZed_Stutter2',
}

function ParasiteZed.VolumeHandler(dist, range)
    if dist >= range then
        return 0
    elseif dist <= 0 then
        return 1
...
function ParasiteZed.isOnAlert(zed)
	if not (ParasiteZed.isParasiteZed(zed) or ParasiteZed.isParasiteQueen(zed)) then return end
	return zed:isCurrentState(ZombieTurnAlerted.instance())
end

function ParasiteZed.playSFX(zed, sfx)
	sfx = sfx or  ParasiteZed.getSFX()
	if not (ParasiteZed.isParasiteZed(zed) or ParasiteZed.isParasiteQueen(zed)) then return end
	local range = 50
	local pl = getPlayer(); if not pl then return end
	if isClient() then
		if ParasiteZed.isClosestPl(pl, zed) or pl == zed then
			local sq = zed:getSquare()
			if sq then
				local vol = ParasiteZed.VolumeHandler(ParasiteZed.checkDist(pl, zed), range)
...
function ParasiteZed.getSFX()
	local list = ParasiteZed.sfxZedList
	return list[ZombRand(1, #list)]
end

function ParasiteZed.preSFX(zed)
	if not (ParasiteZed.isParasiteZed(zed) or ParasiteZed.isParasiteQueen(zed))  then return end
	if zed:getEmitter() then
		local audioName = nil
		local list = ParasiteZed.sfxBanList
		for _, audioName in ipairs(list) do
			if zed:getEmitter():isPlaying(audioName) then
				...
				ParasiteZed.playRandSfx(zed)
			end
		end
	end
end
Events.OnZombieUpdate.Remove(ParasiteZed.preSFX)
Events.OnZombieUpdate.Add(ParasiteZed.preSFX)
...
function ParasiteZed.keypress(key)
	local pl = getPlayer()
	if not ParasiteZed.isParasitePl(pl) then return end
...
Events.OnKeyPressed.Remove(ParasiteZed.keypress)
Events.OnKeyPressed.Add(ParasiteZed.keypress)


function ParasiteZed.OnZombieDead(zed)
	if not (ParasiteZed.isParasiteZed(zed) or ParasiteZed.isParasiteQueen(zed)) then return end
	ParasiteZed.playSFX(zed, "ParasiteZed_Hurt")
end
Events.OnZombieDead.Remove(ParasiteZed.OnZombieDead)
Events.OnZombieDead.Add(ParasiteZed.OnZombieDead)
