...
------------------------               ---------------------------
function ParasiteZed.doRoll(percent) if percent >= ZombRand(1, 101) then return true end return false end
-----------------------            ---------------------------



function ParasiteZed.TicksSinceSeenZombie()
    local pl = getPlayer()
    return pl.TicksSinceSeenZombie >= 6000
end
function ParasiteZed.lastSeenZombieTime()
    local pl = getPlayer()
...
function ParasiteZed.setStats(zed)
    if zed then
        if zed:isReanimatedPlayer() then
            return
        end
        if zed:getModData()['ParasiteZed_Init'] then
            zed:getModData()['ParasiteZed_Init'] = nil
        end
        if ParasiteZed.isParasiteZed(zed) and zed:getModData()['ParasiteZed_Init'] == nil then
...
function ParasiteZed.setStatsToAll()
    local zombies = getCell():getZombieList()
    if not getCell() or not zombies then return end
    for i = 0, zombies:size() - 1 do
        local zed = zombies:get(i)
        if zed and ParasiteZed.isParasiteZed(zed) then
			local num = ParasiteZed.getDayTimeInt()
            ParasiteZed.setTag(zed, num)
...
---------            ---------------------------
--[[
function ParasiteZed.getCol(int)
