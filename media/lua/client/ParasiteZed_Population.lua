...
function ParasiteZed.doDespawn(zed)
    if zed:isAlive() then
        if ParasiteZed.isParasiteQueen(zed) and ParasiteZed.QueenZed == zed  then
...
function ParasiteZed.handleCorpse(corpse)
    if ParasiteZed.isDeadParasite(corpse) or corpse:getModData()['ParasiteZed_Init']  then
        local w = corpse:getWornItems()
        for i = 0, w:size() - 1 do
            local item = w:getItemByIndex(i)
            if item then
                local fType = "Base.ParasiteZed"
                if fType and item:getFullType() == fType then
                    local sq = corpse:getSquare()
                    if sq then
                        sq:AddWorldInventoryItem("Base.ParasiteZed_Corpse", ZombRand(0, 1.5), ZombRand(0, 1.5), 0)
                        ParasiteZed.SpawnRewards(sq)
...
function ParasiteZed.CorpseCleaner()
    local rad = 8
        ...
        for yDelta = -rad, rad do
            ...
            if sq then
                ...
                if corpse and instanceof(corpse, "IsoDeadBody") then
                    ParasiteZed.handleCorpse(corpse)
                end
            end
        end
    end
end
Events.EveryOneMinute.Remove(ParasiteZed.CorpseCleaner)
--Events.EveryOneMinute.Add(ParasiteZed.CorpseCleaner)

function ParasiteZed.isDeadParasite(corpse)
	if not corpse or not instanceof(corpse, "IsoDeadBody") then return false end
    if corpse:getModData()['ParasiteZed_Init'] then return true end
    local fit = tostring(corpse:getOutfitName())
    if fit == "ParasiteZed" then return true end
	return false
end

-----------------------     unused*            ---------------------------

function ParasiteZed.getSpawnRandomZedInfo(fit)
